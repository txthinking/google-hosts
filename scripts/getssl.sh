...
#
echo -e "IP\tLOSS\tTIME\tSSL" > /tmp/getssl;
for((i=0;i<255;i++))
do
    ip=${1}.${i};
    ...
    if [ $c -eq 0 ]
    then
        echo -e "$ip\t$loss\tNO\t$cer" >> /tmp/getssl;
        continue;
    fi
    avgtime=$(grep -E "time=" /tmp/ping | awk '{print $7}' | awk 'BEGIN {FS="=";s=0;c=0;}{s+=$2;c++;} END {print s/c}');
    echo -e "$ip\t$loss\t$avgtime\t$cer" >> /tmp/getssl;
done
sort -k4 -k3n /tmp/getssl
